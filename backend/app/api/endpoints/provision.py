...
async def get_boot_ipxe(mac: str, db: AsyncSession = Depends(get_db)):
    ...
    if box.status == BoxStatus.INSTALLING:
        ...
        if os_type_val == OsType.UBUNTU:
            ...
            # Debian / Other
            cmdline = f"initrd={initrd_file} auto=true priority=critical cdrom-detect/enable=false anna/choose_modules=netboot-retriever console=tty0 vga=788 preseed/url=http://{settings.API_HOST}:{settings.API_PORT}/api/provision/{mac}/preseed.cfg netcfg/choose_interface=auto"
